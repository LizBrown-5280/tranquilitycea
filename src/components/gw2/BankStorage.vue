...
const bankItems = computed(() => gw2.bankItems)
...
function getBankFields(item: (typeof bankItems.value)[number]): UnlockDetailField[] {
  const fields: UnlockDetailField[] = [
    { label: 'Count', value: item.count.toLocaleString(), emphasis: true },
    { label: 'Slot', value: String(item.slotIndex) },
  ]
...
<template>
  <section class="storage-page">
    <header class="storage-page__header">
      ...
      <h2>Bank</h2>
      <p>
        Full bank storage grid with shared item lookup data and relation-expanded hover details.
      </p>
    </header>

    <p v-if="!hasAccountKey" class="storage-page__empty">
      Add an API key with the inventories scope to load bank storage.
    </p>
